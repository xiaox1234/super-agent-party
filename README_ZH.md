...
| THA桌宠机器人 | Live2D风格音频驱动口型动画，实时语音合成与对话打断，透明窗口可叠加到OBS推流！ |
| Vtube Studio机器人 | 控制Vtube Studio中Live2D模型，支持实时表情同步、自定义动作和语音交互！ |
...
5. 本项目是一个独立开发的开源工具。用户在通过本软件调用第三方 API 服务时，应自行遵守相关服务商的使用条款（Terms of Service）。

6. 本软件通过调用第三方大模型生成的任何内容，其准确性、完整性及合规性由模型提供商及用户行为负责，本软件作者不承担因内容产生的任何法律责任。
